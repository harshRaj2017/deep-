...
def postprocess(prediction, num_classes, conf_thre=0.7, nms_thre=0.45, class_agnostic=True):
    ...
    for i, image_pred in enumerate(prediction):
        ...
        # Get scores
        class_conf = image_pred[:, 5]  # cls_conf
        class_pred = torch.zeros_like(class_conf)  # all zero for single class
        
        # Debug prints
        print(f"Prediction shape: {image_pred.shape}")
        print(f"Max confidence - cls_conf: {class_conf.max()}")
        print(f"Mean confidence - cls_conf: {class_conf.mean()}")
        
        # Filter by confidence using class confidence only
        ...
        if class_agnostic:
            nms_out_index = torchvision.ops.nms(
                detections[:, :4],
                detections[:, 5],  # using class confidence for NMS
                nms_thre,
            )
        else:
            # Not needed for single class but kept for compatibility
        ...
        print(f"Number of detections after NMS: {0 if output[i] is None else len(output[i])}")
